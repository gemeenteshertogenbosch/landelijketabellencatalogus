...
class Tabel48Fixtures extends Fixture
{
    ...
    {
    	$csv = fopen(dirname(__FILE__).'/Resources/Tabel48_Nederlands_Reisdocument.csv', 'r');
    	$i = 0;
    	...
    	while (!feof($csv)) {   
	    	...
	    	$entity = New Tabel48();
	    	$entity->setNederlandsReisdocument($line[0]);
	    	$entity->setOmschrijving($line[1]);
	    	if($line[2] && $datumIngang = date_create_from_format('Ymd', $line[3])){$entity->setDatumIngang($datumIngang);}
	    	if($line[3] && $datumEinde = date_create_from_format('Ymd', $line[4])){$entity->setDatumEinde($datumEinde);}
